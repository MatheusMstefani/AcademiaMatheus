...
public class ColaboradorService : IColaboradorService
{
    ...
    public async Task<IEnumerable<ColaboradorDto>> ObterPorTipoAsync(AppColaboradorTipo tipo, CancellationToken cancellationToken = default)
    {
        ...
        return await MapearAsync(await _repoFactory().ObterPorTipo(tipo.ToDomain(), cancellationToken), cancellationToken);
    }
    ...
    {
        if (!Enum.IsDefined(vinculo)) throw new ArgumentOutOfRangeException(nameof(vinculo));
        return await MapearAsync(await _repoFactory().ObterPorVinculo(vinculo.ToDomain(), cancellationToken), cancellationToken);
    }
    ...
    private static async Task ValidarUnicidadeAsync(IColaboradorRepository repo, ColaboradorDto dto,
    ...
    private async Task<ColaboradorDto?> MapearAsync(Colaborador? entity, CancellationToken ct)
    {
    ...
    private async Task<IEnumerable<ColaboradorDto>> MapearAsync(IEnumerable<Colaborador> entities, CancellationToken ct)
    {
